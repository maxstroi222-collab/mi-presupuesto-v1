...
import { Plus, Save, X, Trash2, LogOut, User } from 'lucide-react';
import { 
...
export default function Dashboard() {
  ...
  // CAMPOS DE FORMULARIO LOGIN/REGISTRO
  ...
  // 1. VERIFICAR SI HAY USUARIO AL ENTRAR
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      setSession(session);
      if(session) fetchTransactions(session.user.id);
      else setAllTransactions([]); // Limpiar datos al salir
    });
  ...
  // --- VISTA DE LOGIN (MODIFICADA CON NOMBRE) ---
  if (!session) {
    return (
          ...
          <div className="space-y-4">
            ...
            {authMode === 'register' && (
                <div>
                    ...
                    <input 
                        type="text" 
                        className="w-full bg-[#0d1117] border border-slate-700 rounded p-3 text-white focus:border-emerald-500 outline-none"
            ...
            <button 
                ...
                className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-lg transition-all mt-4"
            >
            ...
            <div className="text-center mt-4">
                <button 
                    onClick={() => {
                        ...
                        setFullName(''); // Limpiar nombre al cambiar modo
                    }}
  ...
  // NOMBRE DE USUARIO (Extraído de los metadatos)
  // Si no tiene nombre (usuarios viejos), usa el email
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 font-sans text-white relative" style={{ backgroundColor: THEME.bg }}>
      ...
      {/* Botones Flotantes */}
      <button onClick={() => setShowForm(true)} className="fixed bottom-8 right-8 z-50 bg-emerald-500 hover:bg-emerald-400 text-black font-bold p-4 rounded-full shadow-2xl transition-all hover:scale-110">
        <Plus size={24} />
      </button>
      ...
      <div className="max-w-[1600px] mx-auto grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <div className="md:col-span-3 space-y-4">
          <div className="bg-[#161b22] p-6 rounded-xl border border-slate-800 text-center">
             <h1 className="text-3xl font-light text-white">{currentMonthName}</h1>
        ...
        <div className="md:col-span-6 flex flex-col gap-4">
            ...
            {/* SECCIÓN DE BIENVENIDA CON NOMBRE REAL */}
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#161b22] px-4 py-5 rounded-xl border border-slate-800 flex flex-col justify-between h-32 relative overflow-hidden">
                    ...
                    <div>
                        <h3 className="text-emerald-400 text-sm font-medium">Ingresos</h3>
                        <h2 className="text-2xl font-bold text-white mt-1">{formatEuro(income)}</h2>
                    </div>
                    <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden mt-2">
                        <div className="bg-emerald-500 h-full" style={{width: `${Math.min((income/2500)*100, 100)}%`}}></div>
                    </div>
                </div>
                ...
                <div className="bg-[#161b22] px-4 py-5 rounded-xl border border-slate-800 flex flex-col justify-between h-32 relative overflow-hidden">
                    ...
                    <div>
                        <h3 className="text-rose-500 text-sm font-medium">Gastos</h3>
                        <h2 className="text-2xl font-bold text-white mt-1">{formatEuro(totalExpenses)}</h2>
                    </div>
                    <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden mt-2">
